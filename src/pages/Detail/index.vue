            ...
            <div class="chooseArea">
              ...
              >
                ...
                <dd
                  ...
                  v-for="item in spuSaleAttr.spuSaleAttrValueList"
                  :key="item.id"
                  @click="checkChange(item, spuSaleAttr.spuSaleAttrValueList)"
                >
            ...
            <div class="cartWrap">
              ...
              <div class="controls">
                <input
                  ...
                  @change="changeNum"
                />
                <button href="javascript:" class="plus" @click="addNum">
                  +
                </button>
                <button
                  href="javascript:"
...
export default {
  ...
  methods: {
    // 将商品添加到购物车，上传数据到服务器，然后跳转到成功添加到购物车路由
    async addToCart() {
      // 解构返回的结果用以判断是否成功
      let {
        data: { code },
      } = await addShopcart(this.skuInfo.id, this.buyNum); // 该接口需要携带两个params参数

      // 失败
      if (code !== 200) return alert("添加购物车失败");

      // 成功
      // 利用会话内存缓存商品详细信息，购物车添加成功路由，需要展示商品信息
      sessionStorage.setItem("skuInfo", JSON.stringify(this.skuInfo));
      // 路由跳转 需要携带购买的数量
      this.$router.push({
        name: "addCart",
        query: { skuNum: this.buyNum },
      });
    },

    // 点击高亮商品参数获取
    checkChange(obj, arr) {
      // 可以直接修改，数据可以响应式，数据变化视图跟这变化
      // obj是点击的商品参数对象，干掉所有人，即将该对象的isChecked保留为 1
      // arr是所有人，里面包含商品的所有参数对象
      arr.forEach((i) => {
        i.isChecked = "0";
      });
      obj.isChecked = "1";
    ...
    // 购买数量的表单元素的change回调
    changeNum() {
      if (this.buyNum < 1) return (this.buyNum = 1); // 最少买一件
    },
    addNum() {
      this.buyNum++;
    },
  ...
  mounted() {
    // 挂载后发送请求，获取商品数据
    this.$store.dispatch("Detail/getDetail", this.$route.params.skuid);
  },
};
</script>
