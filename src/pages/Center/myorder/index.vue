<template>
  <div class="order-right">
    <div class="order-content">
      <div class="title">
        <h3>我的订单</h3>
      </div>
      <div class="chosetype">
        <table>
          <thead>
            <tr>
              <th width="29%">商品</th>
              <th width="31%">订单详情</th>
              <th width="13%">收货人</th>
              <th>金额</th>
              <th>状态</th>
              <th>操作</th>
            </tr>
          </thead>
        </table>
      </div>
      <div class="orders">
        <table class="order-item" v-for="item in records" :key="item.id">
          <thead>
            <tr>
              <th colspan="5">
                <span class="ordertitle"
                  >{{ item.createTime }}&nbsp;&nbsp;订单编号：{{
                    item.outTradeNo
                  }}
                  <span class="pull-right delete"
                    ><img src="../images/delete.png" /></span
                ></span>
              </th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(cate, index) in item.orderDetailList" :key="cate.id">
              <td width="60%">
                <div class="typographic">
                  <img :src="cate.imgUrl" style="width: 100px; height: 100px" />
                  <a href="#" class="block-text">{{ cate.skuName }}</a>
                  <span>x{{ cate.skuNum }}</span>
                  <a href="#" class="service">售后申请</a>
                </div>
              </td>
              <td rowspan="2" width="8%" class="center" v-if="index == 0">
                {{ item.consignee }}
              </td>
              <td rowspan="2" width="13%" class="center" v-if="index == 0">
                <ul class="unstyled">
                  <li>总金额:&nbsp;¥{{ item.totalAmount }}.00</li>
                  <li>在线支付</li>
                </ul>
              </td>
              <td rowspan="2" width="8%" class="center" v-if="index == 0">
                <a href="#" class="btn">{{ item.orderStatusName }} </a>
              </td>
              <td rowspan="2" width="13%" class="center" v-if="index == 0">
                <ul class="unstyled">
                  <li>
                    <a href="mycomment.html" target="_blank">评价|晒单</a>
                  </li>
                </ul>
              </td>
            </tr>
          </tbody>
        </table>
      </div>
      <div class="choose-order">
        <!-- 分页器 -->
        <Pagination
          @jump="jumpPage"
          :pageNo="page"
          :pageSize="limit"
          :total="total"
          :continues="5"
        />
      </div>
    </div>
    <!--猜你喜欢-->
    <div class="like">
      <h4 class="kt">猜你喜欢</h4>
      <ul class="like-list">
        <li class="likeItem">
          <div class="p-img">
            <img src="../images/itemlike01.png" />
          </div>
          <div class="attr">
            <em>DELL戴尔Ins 15MR-7528SS 15英寸 银色 笔记本</em>
          </div>
          <div class="price">
            <em>¥</em>
            <i>3699.00</i>
          </div>
          <div class="commit">已有6人评价</div>
        </li>
        <li class="likeItem">
          <div class="p-img">
            <img src="../images/itemlike02.png" />
          </div>
          <div class="attr">Apple苹果iPhone 6s/6s Plus 16G 64G 128G</div>
          <div class="price">
            <em>¥</em>
            <i>4388.00</i>
          </div>
          <div class="commit">已有700人评价</div>
        </li>
        <li class="likeItem">
          <div class="p-img">
            <img src="../images/itemlike03.png" />
          </div>
          <div class="attr">DELL戴尔Ins 15MR-7528SS 15英寸 银色 笔记本</div>
          <div class="price">
            <em>¥</em>
            <i>4088.00</i>
          </div>
          <div class="commit">已有700人评价</div>
        </li>
        <li class="likeItem">
          <div class="p-img">
            <img src="../images/itemlike04.png" />
          </div>
          <div class="attr">DELL戴尔Ins 15MR-7528SS 15英寸 银色 笔记本</div>
          <div class="price">
            <em>¥</em>
            <i>4088.00</i>
          </div>
          <div class="commit">已有700人评价</div>
        </li>
      </ul>
    </div>
  </div>
</template>

<script>
export default {
  name: "myOrder",
  data() {
    return {
      // 默认当前页为第一页
      page: 1,
      // 每页展示的条目数量,默认展示3条
      limit: 3,
      // 请求我的订单返回数据
      OrderInfo: {},
    };
  },

  computed: {
    // 展示的总数目，注意不是总页数
    total() {
      return this.OrderInfo.total;
    },
    // 每页展示的订单数组列表，默认是每页3笔，每笔可能包含多个商品
    records() {
      return this.OrderInfo.records || [];
    },
  },

  methods: {
    // 请求数据函数
    async getData(page, limit) {
      let {
        data: { code, data, message },
      } = await this.$API.reqMyOrder(page, limit);
      if (code == 200) {
        this.OrderInfo = data;
      } else alert("我的订单数据请求失败" + message);
    },
    // 跳转页面
    jumpPage(index) {
      this.page = index;
      this.getData(this.page, this.limit);
    },
  },
  mounted() {
    // 跳转挂载后直接发请求
    this.getData(this.page, this.limit);
  },
};
</script>

<style>
</style>